...
class PlaneDecoder(nn.Module):
    def __init__(self, in_channels=3600, num_queries=100, d_model=256):
        ...
        self.conv = nn.Sequential(
            ...
            nn.Conv1d(in_channels // 4, in_channels // 8, kernel_size=1),
            nn.ReLU(),
            nn.BatchNorm1d(in_channels // 8),
            nn.Conv1d(in_channels // 8, num_queries, kernel_size=1),
        )
        self.fc = nn.Sequential(
            nn.Linear(d_model, d_model),
            nn.ReLU(),
            nn.BatchNorm1d(num_queries),
            nn.Linear(d_model, d_model),
        )
        self.num_layers = 1
...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        if drop_decoder:
            self.decoder = PlaneDecoder(d_model=d_model, num_queries=num_queries)
        else:
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        if self.two_stage:
            ...
            # modify implementation for two-stage Deformable DETR
...
class NonDeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False):
        ...
        # Modify implementation for iterative bounding box refinement and two-stage Deformable DETR
        self.bbox_embed = None
        self.class_embed = None

    def forward(self, tgt, src, src_padding_mask=None):
        output = tgt
        # Modify: remove reference_points
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            # Modify implementation for iterative bounding box refinement
...
def build_deforamble_transformer(ndim):
