...
class DeepGraphConv_Surv(torch.nn.Module):
    def __init__(self, edge_agg='latent', resample=0, ndim=1024, hidden_dim=256, 
        linear_dim=256, pool=False, use_edges=False, dropout=0.25, n_classes=4):
...
class PatchGCN_Surv(torch.nn.Module):
    def __init__(self, num_layers=4, edge_agg='spatial', multires=False, resample=0,
        fusion=None, ndim=1024, hidden_dim=128, linear_dim=64, use_edges=False, pool=False, dropout=0.25, n_classes=4):
        ...
        if self.resample > 0:
            self.fc = nn.Sequential(*[nn.Dropout(self.resample), nn.Linear(ndim, 256), nn.ReLU(), nn.Dropout(0.25)])
        else:
            self.fc = nn.Sequential(*[nn.Linear(ndim, 128), nn.ReLU(), nn.Dropout(0.25)])
